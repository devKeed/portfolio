  ...
  if (!post) {
    return (
      <Stack spacing={3} className="topSpace innerWidth">
        ...
        <Link to="/blog" style={{ textDecoration: "none", color: theme.palette.primary.main }}>
          ← Back to all posts
        </Link>
      </Stack>
    );
    ...
    <Stack spacing={3} className="topSpace innerWidth">
      ...
      >
        {filteredPosts.map((post) => (
          <motion.div key={post.id} variants={itemVariants} style={{ width: '100%', padding: '12px' }}>
            <Box sx={{ display: 'flex', width: { xs: '100%', sm: '50%', md: '33.33%' } }}>
              <Link to={`/blog/${post.id}`} style={{ textDecoration: 'none', width: '100%' }}>
                <Card
                  sx={{
                    height: '100%',
                    display: 'flex',
